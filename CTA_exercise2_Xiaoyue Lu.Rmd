...
#1. Define the list of newspapers: The variable "newspapers" is used to define the Twitter account names of eight newspapers.
newspapers = c("TheSun", "DailyMailUK", "MetroUK", "DailyMirror", 
               "EveningStandard", "thetimes", "Telegraph", "guardian")

#2. Fetch tweets: The function "get_all_tweets" is called, passing the list of newspapers, start date, and end date as parameters. This function retrieves corresponding tweet data from the Twitter API. The obtained tweet data is stored in the "data/sentanalysis/" directory.
tweets <-
  get_all_tweets(
    users = newspapers,
...
#3. Organize tweet data: The "bind_tweets" function is used to organize the retrieved tweet data, and it is saved in an RDS file.
tweets <- 
...
# The following code blocks perform sentiment analysis on tweet data and visualize sentiment trends using the tidyverse and ggplot2 packages in R.
```{r}
...
mordict <- c('death', 'illness', 'hospital', 'life', 'health',
             'fatality', 'morbidity', 'deadly', 'dead', 'victim')
# 医疗相关的词汇字典
med_dict <- c('health', 'hospital', 'doctor', 'patient', 'medicine',
              'treatment', 'care', 'illness', 'medical', 'wellness')

library(dplyr)

#get total tweets per day (no missing dates so no date completion required)
totals_med <- tidy_tweets %>%
  mutate(obs = 1) %>%
  group_by(date) %>%
...
toks_news_newspaper <- tokens_lookup(toks_news, dictionary = data_dictionary_LSD2015_pos_neg)

```

```{r}
# create a document document-feature matrix and group it by date
dfmat_news_newspaper <- dfm(toks_news_newspaper) %>% 
  dfm_group(groups = docvars(toks_news)$newspaper)
```
...
```{r}
any(is.na(dfmat_news_newspaper))
dfmat_news_newspaper <- na.omit(dfmat_news_newspaper)
unique(dfmat_news_newspaper$newspaper)
```

```{r}
# Convert a dfm object to a data frame
dfmat_news_df <- convert(dfmat_news_newspaper, to = "data.frame")

str(dfmat_news_df)
```

```{r}
colnames(dfmat_news_df)
```

##```{r}
agg_data <- aggregate(. ~ newspaper, data = dfmat_news_df[, c("newspaper", "positive", "negative")], sum)

barplot(t(agg_data[, c("positive", "negative")]), beside = TRUE, 
        col = c("green", "red"), names.arg = agg_data$newspaper,
        ylab = "Frequency", xlab = "Newspaper")
legend("topright", legend = c("Positive", "Negative"), fill = c("green", "red"))

dfmat_news_df$doc_id <- as.numeric(dfmat_news_df$doc_id)
barplot(height = cbind(agg_data$positive, agg_data$negative),
        beside = TRUE, col = c("green", "red"),
        legend.text = rownames(agg_data),
        names.arg = agg_data$newspaper, 
        main = "Sentiment Analysis by Newspaper",
        xlab = "Newspaper",
        ylab = "Frequency")
...
```{r}
column_names <- colnames(dfmat_news_df)
print(column_names)
colnames(dfmat_news_df)[colnames(dfmat_news_df) == "doc_id"] <- "newspaper"

print(colnames(dfmat_news_df))
```

```{r}
labels <- c("Daily Mail U.K.", "Evening Standard", "	
Metro", "The Guardian", "The Mirror", "The Sun", "The Telegraph", "The Times")

matplot(1:nrow(dfmat_news_df), dfmat_news_df[, c("positive", "negative")], type = "l", lty = 1, col = 1:2,
        ylab = "Frequency", xlab = "newspaper")

axis(side = 1, at = 1:nrow(dfmat_news_df), labels = labels)

grid()
legend("topleft", col = 1:2, legend = colnames(dfmat_news_df), lty = 1, bg = "white")
```
```{r}
labels <- c("Daily Mail U.K.", "Evening Standard", "	
Metro", "The Guardian", "The Mirror", "The Sun", "The Telegraph", "The Times")
...
plot(1:nrow(dfmat_news_df), dfmat_news_df[,"positive"] - dfmat_news_df[,"negative"], 
     type = "l", ylab = "Sentiment", xlab = "")
...
```{r}
length(toks_news_lsd)
summary(dfmat_news_lsd)
dfmat_news_lsd <- dfm(toks_news_lsd)
summary(dfmat_news_lsd)
```
```{r}
...
```{r}
negative <- dfmat_news_df$negative
positive <- dfmat_news_df$positive
newspaper <- dfmat_news_df$newspaper

tidy_sent <- as.data.frame(cbind(negative, positive, newspaper))

tidy_sent$negative <- as.numeric(tidy_sent$negative)
tidy_sent$positive <- as.numeric(tidy_sent$positive)
tidy_sent$sentiment <- tidy_sent$positive - tidy_sent$negative
tidy_sent$newspaper <- as.Date(tidy_sent$newspaper)
```

And plot accordingly:

```{r}
summary(tidy_sent$sentiment)
summary(tidy_sent$scaled_sentiment)
str(tidy_sent)
any(!is.finite(tidy_sent$scaled_sentiment))
```

```{r}
tidy_sent$newspaper <- as.character(tidy_sent$newspaper)

tidy_sent %>%
  ggplot() +
  geom_line(aes(newspaper, sentiment, group = 1)) +
  ylab("Sentiment") +
  xlab("newspaper")
...
```{r}
tidy_sent$scaled_sentiment <- tidy_sent$sentiment / 1000

tidy_sent %>%
  ggplot() +
  geom_line(aes(newspaper, scaled_sentiment)) +
